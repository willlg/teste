...
function initializePowerUp() {
  ...
  window.TrelloPowerUp.initialize({
    'board-buttons': function (t, options) {
      ...
      window.BRProjectUtils.initializeBrowserCloseHandler(t);
      window.BRProjectUtils.checkForUnfinishedTaskWarning();
      return [{
        icon: ICON_BR,
        text: 'BRProject',
        callback: function (t) {
...
window.BRProjectUtils = {
  ...
  clearUserData: async function(t) {
   ...
   initializeBrowserCloseHandler: function(t) {
    const self = this;
    
    async function checkForRunningTasks() {
      try {
        const { token, url } = await self.getUserData(t);
        ...
        console.error('Erro ao verificar tarefas em andamento:', e);
        return null;
      }
    }
    ...
    async function stopRunningTask() {
      try {
        ...
        console.error('Erro ao parar tarefa:', e);
        return false;
      }
    }
    
    window.addEventListener('beforeunload', async function(e) {
      const runningTask = await checkForRunningTasks();
      
      if (runningTask) {
        const message = `Você tem uma tarefa em andamento: "${runningTask.tarefa.nome}". Deseja finalizá-la antes de sair?`;
    ...
    document.addEventListener('visibilitychange', async function() {
      if (document.visibilityState === 'hidden') {
        ...
        if (runningTask) {
          console.log('Página oculta com tarefa em andamento:', runningTask.tarefa.nome);
          
          localStorage.setItem('brproject-task-warning', JSON.stringify({
            taskName: runningTask.tarefa.nome,
            taskId: runningTask.tarefa.idtarefa,
            timestamp: Date.now()
          }));
        }
      }
  ...
  checkForUnfinishedTaskWarning: function() {
    const warning = localStorage.getItem('brproject-task-warning');
      ...
      try {
        ...
        if (timeDiff < 5 * 60 * 1000) {
          const shouldFinish = confirm(
            `Na sua última sessão, você tinha uma tarefa em andamento: "${data.taskName}"\n\n` +
            `Deseja finalizá-la agora?`
          );
          
          if (shouldFinish) {
            console.log('Usuário escolheu finalizar a tarefa:', data.taskName);
          }
        }
        
        localStorage.removeItem('brproject-task-warning');
        
      } catch (e) {
        console.error('Erro ao processar aviso de tarefa:', e);
        localStorage.removeItem('brproject-task-warning');
      }
    }
  },
