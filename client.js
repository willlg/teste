...
async function checkRunningTask(t) {
  try {
    ...
    if (boardTaskStatus) {
      console.log('Status da tarefa no board:', boardTaskStatus);
      return boardTaskStatus.isRunning === true;
    }
    ...
    for (const card of cards) {
      ...
      if (cardStatus === 'running') {
        await t.set('board', 'shared', 'brproject-running-task', {
          isRunning: true,
          cardId: card.id,
          timestamp: Date.now()
        });
    ...
    console.error('Erro ao verificar tarefa em andamento:', error);
    return false;
  }
}
...
function initializePowerUp() {
  ...
  window.TrelloPowerUp.initialize({
    'board-buttons': async function (t, options) {
      ...
      console.log('Status da tarefa em andamento:', hasRunningTask);
